...
bool SMPL::Forward(const beta_t &beta,
                   const theta_t &theta,
                   joint_t *joints,
    ...
    for (size_t i = 0; i < JOINT_NUM; i++) {
        const Eigen::Vector3d theta_i = theta.segment<3>(i * 3);
    ...
    ///> JOINT_NUM_EXTRA x 16 (4 x 4 trafo)
    ...
    vertex_t transformedVertexJoints;
    ...
    for (size_t j = 0; j < JOINT_NUM_EXTRA; j++) {
        ...
        transformedVertexJoints.segment<3>(j * 3) = (tf * unposedVertex).head<3>();
    }
...
bool SMPL::Forward(const beta_t & beta,
    ...
    if(joints != nullptr) {
        joint_t jointsShift = translation.replicate<JOINT_NUM, 1>();
        *joints += jointsShift;
    }
    if(vertices != nullptr) {
        vertex_t vertexShift = translation.replicate<JOINT_NUM_EXTRA, 1>();
